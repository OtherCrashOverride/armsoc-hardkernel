		...
		unsigned int h)
{
	...
	g2d_add_cmd(ctx, DST_SELECT_REG, G2D_SELECT_MODE_BGCOLOR);	//G2D_SELECT_MODE_BGCOLOR //G2D_SELECT_MODE_NORMAL
	...
	flip = 0;
	if (src->x_reverse_flag != 0)
	{
		flip |= 1;
	}

	if (src->y_reverse_flag != 0)
	{
		flip |= 2;
	}

	g2d_add_cmd(ctx, SRC_MASK_DIRECT_REG, flip);


	flip = 0;
	if (dst->x_reverse_flag != 0)
	{
		flip |= 1;
	}

	if (dst->y_reverse_flag != 0)
	{
		flip |= 2;
	}

	g2d_add_cmd(ctx, DST_PAT_DIRECT_REG, flip);
